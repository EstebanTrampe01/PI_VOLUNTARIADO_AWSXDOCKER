...
# Rutas base
BASE_DIR="/DOCKER"
MODULO_PROCESOS_DIR="/DOCKER/Modulos_Kernel"
...
for modulo in "${MODULOS[@]}"; do
    ...
    if [ -f "$KO_FILE" ]; then
        ...
        if [ -f "/proc/${modulo}_202200314" ]; then
            ...
            echo " Error --> No se encontró /proc/${modulo}_202200314"
        fi
    else
        echo " Error --> No se generó el archivo $KO_FILE"
    fi
...
if [ -f "procesos_202200314.ko" ]; then
    ...
    if [ -f "/proc/procesos_202200314" ]; then
        ...
        echo " Error --> No se encontró /proc/procesos_202200314"
    fi
else
    echo " Error --> No se generó procesos_202200314.ko"
fi
